...
// ---- Robust mode & player-count based rule for hiding Friends panel ----
const modeRaw =
  (frame as any)?.mode ??
  (frame as any)?.gameMode ??
  (frame as any)?.roomType ??
  (frame as any)?.type ??
  (frame as any)?.variant ??
  null;
...
const isSingleMode =
  !!mode &&
  (mode.includes("single") || mode.includes("solo"));

const isClassicMode =
  !!mode && mode.includes("classic");

// Hide when single/solo, classic (your request), or only 1 player present
const hideFriendsPanel = isSingleMode || isClassicMode || playerCount <= 1;
// -----------------------------------------------------------------------
    ...
    >
      <div className="w-full max-w-4xl space-y-6">
        ...
        <div className={`grid grid-cols-1 ${hideFriendsPanel ? "sm:grid-cols-2" : "sm:grid-cols-3"} gap-4`}>
          ...
          {/* Friends panel — hidden for single/classic */}
          {!hideFriendsPanel && (
            <div className="flex flex-col bg-gradient-to-br from-cyan-700/90 to-cyan-800/90 backdrop-blur-md border-4 border-cyan-400 rounded-2xl p-8 min-h-[100px] flex items-center justify-center shadow-[0_0_30px_rgba(34,211,238,0.3)]">
              ...
              <div className="flex flex-col w-full mt-2">
                <div className="flex flex-row space-x-4 grow w-full">
                  {otherPlayers.thinking.map((p) => {
                    return (
                      <motion.div layout="position" layoutId={`players-${p.id}`} key={p.id}>
                        <img
                          src={p.avatarUri}
                          alt={`profile-${p.id}`}
                          width={32}
                          height={32}
                          className={`rounded-full outline-3 ${p.roundStatus === "solved" ? "outline-cyan-400" : "outline-gray-400"}`}
                        />
                      </motion.div>
                    )
                  })}
                    ...
                    return (
                      <motion.div layout="position" layoutId={`players-${p.id}`} key={p.id}>
                        <img
                          ...
                          width={32}
                          height={32}
                          className={`rounded-full outline-3 ${p.roundStatus === "solved" ? "outline-green-400" : "outline-gray-400"}`}
                        />
                      </motion.div>
                    )
                  })}
