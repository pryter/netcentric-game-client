...
// 🟦 Square Button (unchanged)
export const SquareButton: FC<ButtonProps> = ({
                                                  className = "px-6 py-4 text-white",
                                                  children,
                                                  onClick,
                                              }) => {
    return (
        <motion.button onClick={onClick} className="relative pt-[6px] cursor-pointer">
            <div className={classNames(className, "relative z-[1] bg-sky-600 outline-4 outline-white")}>
                {children}
            </div>
            <motion.div
                whileHover={{ y: 4 }}
                whileTap={{ y: 8, scaleY: 0.92 }}
                className={classNames(
                    className,
                    "absolute top-0 z-[1] bg-sky-400 outline-white"
                )}
            >
                {children}
            </motion.div>
    ...
    return (
        <motion.button onClick={onClick} className="relative pt-[6px] cursor-pointer">
            <div
                className={classNames(
                    className,
                    "relative z-[1] bg-[#08B9FF] overflow-clip outline-4 outline-white"
                )}
            >
                {children}
            </div>
            <motion.div
                ...
                className={classNames(
                    className,
                    "absolute top-0 overflow-clip z-[1] bg-[#94E7FC] outline-white"
                )}
            >
                <div className="absolute h-full w-1/2 top-0 right-0">
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                        className="absolute h-full top-0 left-[-10px]"
                        viewBox="0 0 56 30"
                        fill="none"
                    >
                        <path
                            d="M48.042 0C52.4601 0.000173171 56.042 3.58183 56.042 8V23.3545C56.0418 27.7725 52.46 31.3543 48.042 31.3545H27.6016L0.379883 0H48.042Z"
                            fill="#4CDAFE"
                        />
                    </svg>
...
// 💜 GameButton (fixed syntax, same logic)
export const GameButton = (props: ButtonProps) => {
    ...
    const colorPalette = useMemo(() => colorCombination[color ?? "purple"], [color]);
    ...
    return (
        <motion.button
            ...
            style={{
                boxShadow: `0 4px 0 color-mix(in oklab, ${colorPalette["600"]} 80%, transparent)`,
            }}
            animate={{
                borderColor: colorPalette["400"],
                background: `linear-gradient(45deg, ${colorPalette["500"]}, ${colorPalette["600"]})`,
            ...
            className={classNames(
                defaultCN,
                `cursor-pointer bg-gradient-to-br border-2 transition-all duration-200 
