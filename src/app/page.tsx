...
import {RoundedButton, SquareButton} from "@/components/Button";
    ...
    <div style={{background: "radial-gradient(50% 50% at 50% 50%, #A659FE 0%, #6F53FD 100%);"}} className="flex flex-col items-end h-screen relative w-full">
      <Modal state={state}>
        <div className="flex flex-col bg-yellow-500 min-w-[300px] border-4 rounded-2xl border-yellow-600 py-4 px-6 z-100">
          ...
          <RoundedButton onClick={sendUserName} className="py-2 text-white">
            Submit
          </RoundedButton>
        </div>
      </Modal>
      ...
      </div>
      {
        !isAuthenticated() && <div className="flex w-full flex-col items-center grow space-y-6 h-[50vh] justify-start">
              <RoundedButton className="py-4 px-6" onClick={signIn} >
                  Sign In With Google
              </RoundedButton>
              <RoundedButton>
      ...
      {isAuthenticated() && <div className="fixed bottom-0 right-0 p-6">
          <div className="flex flex-row space-x-4 rounded-[16px] px-4">
              <Image src={"/assets/sign-bg.png"} className="z-0" alt={"sign"} fill={true}/>
              ...
              <div className="space-y-1 z-1">
                  ...
                  <RoundedButton onClick={() => {
                    signOut()
                  }} className="text-sm py-1.5 px-4 text-white">
                      Logout
                  </RoundedButton>
              </div>
          </div>
